...
- iPad 进入 App 专属文件工作区：左侧保留当前文件列表，右侧预览可在 60%–90%
  之间拖动调整，也可全屏打开。
- 导出前可查看文件元数据和 SHA-256。
- 先将文件暂存到 Vellune 缓存，再调用系统共享面板。
- 可将当前目录导出为 Markdown，并可选择是否递归列出子目录。
- 浏览默认只读；明确执行替换时，会先创建版本化备份、校验 SHA-256，并支持在
  恢复前再次创建安全备份。
...
浏览、预览、搜索和导出均为只读。唯一的写入流程是用户明确确认后，对当前所选
单个文件执行替换。Vellune 会把原文件保存到自己的版本化备份区，使用 SHA-256
校验写入，并在恢复旧版本前再次创建安全备份。备份可以降低风险，但不能替代完整
的设备备份。诊断功能会明确报告失败，而不会假定某条路径可以访问。
