...
- 完整恢复前会校验 Bundle ID、声明的归档内容、文件大小和全部 SHA-256，并先为
  当前容器创建安全备份；替换使用临时 staging 目录，目录交换失败时自动回滚。
...
Markdown 导出、安全编辑、完整 App 备份、可回滚恢复及本地搜索。结果会写入 Vellune 自身数据容器内的
...
会先创建可恢复 ZIP。完整 App 恢复会验证 manifest 和每个文件的哈希，保存当前
