...
int64_t bad_query(char *path, bool create, char *group_identifier, bool is_group) {
    ...
    void *manager = dlopen("/usr/lib/system/libsystem_containermanager.dylib", RTLD_NOW | RTLD_LOCAL);
    if (!manager) return -1;

    container_query_create_fn query_create = (container_query_create_fn)dlsym(manager, "container_query_create");
    container_query_set_class_fn query_set_class = (container_query_set_class_fn)dlsym(manager, "container_query_set_class");
    container_query_set_identifiers_fn query_set_group_identifiers = (container_query_set_identifiers_fn)dlsym(manager, "container_query_set_group_identifiers");
    container_query_set_flags_fn query_set_flags = (container_query_set_flags_fn)dlsym(manager, "container_query_operation_set_flags");
    container_query_set_part_fn query_set_part = (container_query_set_part_fn)dlsym(manager, "container_query_operation_set_part");
    container_query_set_part_domain_fn query_set_part_domain = (container_query_set_part_domain_fn)dlsym(manager, "container_query_operation_set_part_domain");
    container_query_get_single_result_fn query_get_single_result = (container_query_get_single_result_fn)dlsym(manager, "container_query_get_single_result");
    container_query_free_fn query_free = (container_query_free_fn)dlsym(manager, "container_query_free");
    container_copy_sandbox_token_fn copy_sandbox_token = (container_copy_sandbox_token_fn)dlsym(manager, "container_copy_sandbox_token");
...
char *bad_query_list(char *path, int64_t max_inode) {
    if (!path || path[0] != '/' || max_inode < 1) return NULL;

    struct statfs stats;
    if (statfs(path, &stats) != 0) return NULL;
    fsid_t fsid = stats.f_fsid;
    ...
    for (uint64_t inode = 1; inode <= (uint64_t)max_inode; inode++) {
        ...
        if (strncmp(candidate, path, path_length) != 0 || candidate[path_length] != '/') continue;
        ...
        length += (size_t)snprintf(output + length, capacity - length, "%s\n", candidate);
    }
    return output;
}
